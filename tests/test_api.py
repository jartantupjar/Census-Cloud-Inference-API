...
def test_intro_api():
    ...
    assert r.json()== {'message': 'this api is used for generating salary predictions'}
...
def test_incorrect_workclass():
    ...
    assert r.status_code==422
    assert json.loads(r.content)['detail'][0]['type'] == "value_error.str.regex"
...
def test_incorrect_age():
    ...
    assert r.status_code==422
    assert json.loads(r.content)['detail'][0]['type'] == "value_error.number.not_le"
